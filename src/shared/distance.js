export default class Distance {
  ...
  static getLatLonFromDistanceKm(lat,lon,distance){
    ...
    const new_long = lon + coef / Math.cos(new_lat * Math.PI/180);
  ...
  static getLatLonFromDistanceMi(lat,lon,distance){
    const coef = distance * (1/69.172);
    const new_lat = lat + coef;
    const new_long = lon + coef / Math.cos(new_lat * Math.PI/180);
    return {latitude : new_lat , longitude : new_long}
   }
